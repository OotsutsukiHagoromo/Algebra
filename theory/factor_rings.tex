...
\begin{definition}
	Гомоморфизм колец $\phi \colon A \to B$~"--- это отображение подлежащих множеств, которое переводит сложение в сложение, а умножение~"--- в умножение: для любых $a'$, $a'' \in A$ выполняется
	\begin{align*}
		\phi(a' \plusA a'') &=
		\phi(a') \plusB \phi(a''),
		\\
...
\begin{definition}
	Пусть $A$~"--- кольцо. Подмножество $B \subset A$ называется \emph{подкольцом}, если оно замкнуто относительно операций $\bound{+}_B$ и $\bound{\cdot}_B$~"--- ограничений соответствующих операций в~$A$. Иными словами, $B$~"--- подкольцо, если для всех $b_1$, $b_2 \in B$ выполняется
	\begin{align*}
		b_1 \cdot b_2 &\in B,
		\\
		b_1 + b_2 &\in B,
...
\begin{remark}
	...
	\begin{align*}
		...
		\frac{A}{I}
	\end{align*}
	--- действительно \emph{отображения}, то есть всюду определённые и функциональные отношения%
	\footnote{%
...
\begin{definition}
	Гомоморфизм $\phi \colon A \to B$ называется \emph{изоморфизмом}, если существует обратный гомоморфизм $\psi \colon B \to A$, такой, что $\phi \circ \psi = \id_{B}$, $\psi \circ \phi = \id_{A}$, где $\id_X$~"--- тождественный гомоморфизм на~$X$, переводящий~$x \in X$ в~$x$. Иными словами, для любых $a \in A$ и $b \in B$
	\begin{align*}
		(\psi \circ \phi)(a) &= a,
		\\
		(\phi \circ \psi)(b) &= b.
...
Иногда для~$[a] \in \ZZ/(n)$ будем писать~$[a]_n$, чтобы подчеркнуть, по модулю какого~$n$ берётся кольцо вычетов.
...
Пусть $\phi \colon A \to B$~"--- гомоморфизм колец. Тогда \emph{ядро гомоморфизма~$\phi$}~"--- это прообраз нуля:
\[
	\ker{\phi} := \phi^{-1}(0_B) =
	\set{a \in A}{\phi(a) = 0_B};
\]
\emph{образ гомоморфизма~$\phi$}~"--- это образ кольца~$A$:
\[
...
\end{proposition}
\begin{proofm}
	Покажем, что $\ker{\phi}$~"--- подкольцо в~$A$. Для этого покажем, что для любых $x,\ y \in \ker{\phi}$ выполняются условия $x + y \in \ker{\phi}$ и $x \cdot y \in \ker{\phi}$.
	
	Если $x,\ y \in \ker{\phi}$, то $\phi(x) = 0_B$, $\phi(y) = 0_B$. Тогда
	\begin{align*}
		\phi(x + y) &:=
		\phi(x) + \phi(y)
		=
	...
	Аналогично покажем, что~$\im{\phi} \leq B$. Пусть $p,\ q \in \im{\phi}$. Покажем, что тогда $p+q \in \im{\phi}$ и $pq \in \im{\phi}$.

	Так как $p\in \im{\phi}$, то существует (возможно, не единственный) элемент $x \in A$, такой, что $\phi(x) = p$; аналогично, для~$q \in \im{\phi}$ существует элемент $y \in A$, такой, что $\phi(y) = q$. Тогда
	\begin{align*}
		\phi(x+y) &= \phi(x) + \phi(y) = p + q,
		\\
		\phi(xy) &= \phi(x) \phi(y) = pq
		.
	\end{align*}
	Поэтому~$x + y$ --- прообраз~$p+q$, а~$xy$~"--- прообраз элемента~$pq$, так что $p+q \in \im{\phi}$ и $pq \in \im{\phi}$.
\end{proofm}
...
\[
	(a_0, a_1, a_2, \dots),
\]
в которых только конечное число элементов $a_\nu \in K$ отлично от нуля. Такое множество обозначается символом
\raisebox{0pt}[0pt][0pt]{$
	\bigoplus\limits_{\nu \geq 0} K
$}.
Для удобства такие последовательности записываются в виде
\[
...
\begin{definition}
	Полином $f \in K[x]$ называется \emph{приведённым}, если коэффициент при старшей степени~$n = \deg{f}$ равен~$1$: $a_n = 1$.
\end{definition}
...
Поэтому сложение и умножение классов в~$\kk[x]/(f)$ можно формально интерпретировать как сложение и умножение <<многочленов>> вида
\[
	a_0 + a_1 \theta + \dots + a_{n-1} \theta^{n-1}
\]
...
% \input{theory/example_inverse.tex}
	...
	\begin{enumerate}
		...
		\item Теперь пусть $n \in \NN$ и для всех многочленов степени, меньшей $n$, мы умеем строить нужное поле.
			\begin{itemize}
				...
				\item Теперь пусть $f$ неприводим. Рассмотрим поле $\LL := \kk[x]/(f)$. Оно содержит~$\kk$ в качестве классов констант, и~$f$ делится в~$\LL[x]$ на~$x - \theta$, где $\theta = x \bmod f$: $f(x) = (x - \theta) q(x)$ в~$\LL[x]$, причём $\deg{q} = n - 1$, и по индукции~$q$ раскладывается на линейные множители над некоторым полем~$\FF \supset \LL$.
			\end{itemize}
	\end{enumerate}
	Теорема доказана.
\end{proof}
