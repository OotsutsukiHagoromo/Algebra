...
\begin{proposition}
	Обратимость элемента~$[a] \in \ZZ/(n)$ равносильна тому, что $\gcd{a, n} = 1$, так как если существует~$[b] \in \ZZ/(n)$, такой, что $[a][b] = [1]$, то
	\begin{align*}
		ab + (n) &= 1 + (n),
		\\
		ab + kn &= 1 + \ell n,
...
\end{proposition}
	...
	Поэтому если~$p$~"--- простое, то для любого целого~$0 < a < p$ имеем
	\[
		\gcd{a, p} = 1
		,
	\]
...
\begin{proofm}
	...
	Пусть $x,\ y \in \ker{\phi}$. Это значит, что $\phi(x) = 1_B$, $\phi(y) = 1_B$. Тогда
	\[
		\phi(x \cdot y) :=
		\phi(x) \cdot \phi(y)
		=
...
\begin{proof}
	...
	\begin{align*}
		...
		[g][q] &= [1],
	\end{align*}
	и произвольный $[g] \in \bigl( \kk[x]/(f) \bigr) \setminus \set*{[0]}$ обратим.
\end{proof}
...
В случае поля~$\FFp$ имеем~$\Phi \colon \FFp \to \FFp \colon x \mapsto x^p$.
