...
\begin{proof}
	...
	Итак, найдём проекцию~$v$ на~$\pi^\bot$ (обозначим её $v_\bot$), а затем из~$v$ и~$v_\bot$ найдём длину проекции~$v_\pi$ вектора~$v$ на~$\pi$. Для этого воспользуемся многомерной теоремой Пифагора. Так как~$v_\pi \in \pi$ и $v_\bot \in \pi^\bot$, а $\pi \bot \pi^\bot$, то~$v_\pi \bot v_\bot$, и поэтому
	\[
		\norm{v}^2 = \norm{v_\pi}^2 + \norm{v_\bot}^2.
	\]
	...
	\[
		...
		\frac{1}{\det{G}}\,G^\vee,
	\]
	где~$G^\vee$ --- матрица алгебраических дополнений. Найдём
	\[
		\det{G} =
		15 \cdot 3 - 16 =
		(16-1) \cdot 3 - 16
